...
export default function DashboardPage() {
  ...
  // حالة النافذة المنبثقة للبالغين سن 60 قريباً
  ...
  // 🎯 دالة استخراج تاريخ الميلاد وبلوغ سن الـ 60 من الرقم القومي (14 رقم)
  const getAge60Info = (nationalId: string) => {
    ...
    const daysUntil60 = Math.ceil(
      (age60Date.getTime() - today.getTime()) / (1000 * 3600 * 24)
    );
  ...
  const companiesList = Array.from(
    new Set(allEmployees.map((e) => e.company).filter(Boolean))
  );
  ...
  const dashboardData = useMemo(() => {
    ...
    filteredEmps.forEach((emp) => {
      ...
      // حساب من يقترب من سن الـ 60 (خلال 60 يوماً ولم يتغير عقده لـ فوق السن بعد)
      ...
      // حساب عقود الانتهاء (استثناء العقد الدائم)
      if (type !== 'دائم') {
    ...
    // 🌟 حساب معدل التجديدات الفعلي للسنة الحالية (مبني على أول تاريخ متاح لبداية العقد الجديد)
    ...
    filteredRens.forEach((req) => {
      // قبول طلبات التجديد المعتمدة الفعالة (نتحمّل أكثر من تسمية محتملة لنفس الحالة)
      const isApproved =
        req.status === 'Approved' ||
        req.status === 'معتمد' ||
        req.action === 'تجديد' ||
      ...
      if (isApproved) {
        // الاعتماد على تاريخ بداية العقد الجديد، ولو لم يوجد نرجع لتاريخ الطلب كمرجع تقريبي
  ...
  return (
    <div className="flex flex-col gap-5" style={{ direction: 'rtl' }}>
      ...
      <div className="card flex flex-col lg:flex-row lg:items-center justify-between gap-4 px-5 sm:px-6 py-5">
        <div>
          <h2
            className="m-0 text-lg sm:text-xl font-extrabold tracking-tight"
          ...
          >
            ...
            <span className="font-mono" style={{ color: 'var(--brass-600)' }}>
              ⏰ {timeFormatted}
            </span>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        {/* 📈 معدل التجديدات المعتمدة للسنة الحالية */}
        <div className="card px-5 sm:px-6 py-5 flex flex-col">
          <h4
            className="m-0 mb-5 text-[13.5px] font-extrabold"
            style={{ color: 'var(--navy-950)' }}
          >
            📈 معدل التجديدات المعتمدة لعام {dashboardData.targetYear} (بداية العقد الفعلي)
          </h4>
          <div
          ...
          >
            {dashboardData.renewalsByMonth.map((month, idx) => {
              const height =
                maxMonthCount > 0 ? (month.count / maxMonthCount) * 100 : 0;
              return (
                <div
                  key={idx}
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      ...
                      background:
                        month.count > 0
                          ? 'linear-gradient(180deg, var(--brass-400), var(--brass-600))'
                          : 'var(--paper)',
                    }}
                  />
                  <span
            ...
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  ...
                  boxShadow: '0 0 10px rgba(31,92,58,0.35)',
                }}
              />
              <div
            ...
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  ...
                  boxShadow: '0 0 10px rgba(31,63,102,0.35)',
                }}
              />
              <div
                className="flex-1 text-[12px] font-bold"
            ...
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  ...
                  boxShadow: '0 0 10px rgba(184,147,74,0.35)',
                }}
              />
              <div
                className="flex-1 text-[12px] font-bold"
