...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      try {
        ...
        setRawEmployees(mergedEmployees);
        setAllRenewals(renRes.data || []);
      } catch (err) {
  ...
  const [ageModalSelectedMonth, setAgeModalSelectedMonth] = useState(String(new Date().getMonth() + 1));
  const [ageModalSelectedYear, setAgeModalSelectedYear] = useState(String(new Date().getFullYear()));
  ...
  const dashboardData = useMemo(() => {
    ...
    const activeEmployeesOnly = allEmployees.filter((emp: any) => {
      const employeeStatus = normalizeStatus(firstValue(emp?.status, emp?.Status, 'Active'));
      const department = getDepartment(emp);
      return employeeStatus === 'active' && department !== 'تحويلات تحت الاعتماد';
    });

    const normalizedCompanyFilter = normalizeText(filterCompany);
    const normalizedDeptFilter = normalizeText(filterDept);

    const filteredEmps = activeEmployeesOnly.filter((emp: any) => {
      const company = normalizeText(getCompany(emp));
      const department = normalizeText(getDepartment(emp));
      return (
        (!normalizedCompanyFilter || company.includes(normalizedCompanyFilter)) &&
        (!normalizedDeptFilter || department.includes(normalizedDeptFilter))
      );
    });

    const filteredRens = allRenewals.filter((req: any) => {
      const company = normalizeText(firstValue(req?.company, req?.Company));
      const department = normalizeText(firstValue(req?.department, req?.Department));
      return (
        (!normalizedCompanyFilter || company.includes(normalizedCompanyFilter)) &&
        (!normalizedDeptFilter || department.includes(normalizedDeptFilter))
      );
    ...
    const renewalsByEmployee = new Map<string, any[]>();
    filteredRens.forEach((req: any) => {
      const code = getRenewalEmployeeCode(req);
      if (!code) return;
      const current = renewalsByEmployee.get(code);
      if (current) current.push(req);
      else renewalsByEmployee.set(code, [req]);
    });
    ...
    const contractsByMonth = MONTH_NAMES.map((name) => ({
      name,
      count: 0,
      emps: [] as any[],
    }));
    ...
    filteredEmps.forEach((emp: any) => {
      ...
      const contractStatus = normalizeStatus(getContractStatus(emp));
      const nationalId = getNationalId(emp);
      const mobile = getMobile(emp);
      const contractStart = getContractStart(emp);
      const contractEnd = getContractEnd(emp);
      ...
      const empRens = employeeCode ? renewalsByEmployee.get(employeeCode) || [] : [];
      const approvedRens = empRens.filter(isApprovedRenewal);

      const isPermanent = typeNormalized === 'دائم' || typeNormalized === 'permanent' || typeNormalized.includes('غير محدد');
      const isReward = typeNormalized.includes('مكافأة') || typeNormalized.includes('مكافاه') || typeNormalized.includes('reward');
      const isAboveAgeType = typeNormalized.includes('فوق السن') || typeNormalized.includes('above age');

      const ageInfo = getAge60Info(nationalId);

      if (isPermanent && ageInfo) {
      ...
      if (typeNormalized === 'محدد المدة' || typeNormalized === 'fixed term' || typeNormalized === 'fixed_term' || !typeNormalized) {
        if (approvedRens.length > 0) {
          const sortedApproved = [...approvedRens].sort((a, b) => {
            ...
            const dateB = parseDate(firstValue(b?.request_date, b?.RequestDate, b?.created_at, b?.CreatedAt));
            return (dateA?.getTime() || 0) - (dateB?.getTime() || 0);
          });
          const lastRen = sortedApproved[sortedApproved.length - 1];
          const lastRenewalMonths = getRenewalMonths(lastRen);

          if (lastRenewalMonths !== null && lastRenewalMonths > 0 && lastRenewalMonths < 12) {
            isShort = true;
            const historyArr = sortedApproved.map((r: any) => {
              const months = getRenewalMonths(r);
              return months !== null ? `${months} ش` : null;
            }).filter(Boolean);

            historyDesc = historyArr.length > 0 ? `سجل التجديدات: (${historyArr.join(' + ')})` : 'تجديد قصير المدة';
          }
      ...
      if (isPermanent) permEmps.push(emp);
      else if (isReward) rewardEmps.push(emp);
      else if (isAboveAgeType) aboveAgeEmps.push(emp);
      else if (isShort) {
      ...
      if (contractStart && contractStatus === 'active') {
        ...
        if (startDate && startDate.getFullYear() === currentYear) {
          ...
          if (monthIndex >= 0 && monthIndex < 12 && isWorkedContract) {
            contractsByMonth[monthIndex].count++;
            contractsByMonth[monthIndex].emps.push(emp);
          }
        }
      }

      if (contractStatus === 'active') {
        const days = getDaysRemaining(contractEnd);
        if (days !== null) {
    ...
    const shortTermList = Object.entries(shortTermByDept).map(([deptName, emps]) => ({
      deptName,
      emps: [...emps].sort((a, b) => (getDaysRemaining(getContractEnd(a)) ?? 9999) - (getDaysRemaining(getContractEnd(b)) ?? 9999)),
    })).sort((a, b) => b.emps.length - a.emps.length);

    const topDepts = Object.entries(deptsCount).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([name, count]) => ({ name, count }));
    const pendingRequests = filteredRens.filter(isPendingRenewal);
    const waitingSign = filteredRens.filter((r: any) => {
      const status = normalizeText(firstValue(r?.status, r?.renewal_status, r?.RenewalStatus));
      const signatureStatus = normalizeText(firstValue(r?.signature_status, r?.SignatureStatus));
      return (status === 'approved' || status === 'معتمد') && signatureStatus !== 'تم التوقيع';
    });
    ...
    return {
      ...
      permEmps, fixedEmps, rewardEmps, aboveAgeEmps, shortTermEmpsList,
  ...
  const displayTurning60List = useMemo(() => {
    return dashboardData.allTurning60List.filter((emp: any) => {
      if (ageModalFilterMode === '60days') return emp.daysLeft >= 0 && emp.daysLeft <= 60;
      ...
      if (ageModalFilterMode === 'allYear') return !ageModalSelectedYear || emp.age60Year === ageModalSelectedYear;
      return true;
    });
  }, [dashboardData.allTurning60List, ageModalFilterMode, ageModalSelectedMonth, ageModalSelectedYear]);

  const handleRowClick = (rawCode: any, targetPage: 'contracts' | 'employees' = 'contracts', employee?: any) => {
    ...
    const employeeId = String(firstValue(employee?.employee_id, employee?.EmployeeID, employee?.employeeId, '')).trim();
    const employeeCode = getEmployeeCode(employee) || code;

    localStorage.setItem('selectedEmployeeCode', employeeCode);
    localStorage.setItem('employeeSearch', employeeCode);
    localStorage.setItem('jumpSearch', employeeCode);
  ...
  const p1 = totalContracts > 0 ? (dashboardData.permCount / totalContracts) * 100 : 0;
  const p2 = p1 + (totalContracts > 0 ? (dashboardData.fixedCount / totalContracts) * 100 : 0);
  const p3 = p2 + (totalContracts > 0 ? (dashboardData.rewardCount / totalContracts) * 100 : 0);
  const p4 = p3 + (totalContracts > 0 ? (dashboardData.aboveAgeCount / totalContracts) * 100 : 0);
  ...
  return (
    <div className="flex flex-col gap-5" style={{ direction: 'rtl', paddingBottom: '40px' }}>
      ...
      <div className="card flex flex-col lg:flex-row lg:items-center justify-between gap-4 px-5 sm:px-6 py-5" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px' }}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <datalist id="dashCompList">
            {companiesList.map((c: any, i: number) => <option key={i} value={c} />)}
          </datalist>
          ...
          <datalist id="dashDeptList">
            {deptsList.map((d: any, i: number) => <option key={i} value={d} />)}
          </datalist>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3">
        ...
        <KpiCard loading={loading} tone="blue" title=" نماذج تحت الاعتماد" value={dashboardData.pendingCount} sub={`+ ${dashboardData.waitingSignCount} توقيع`} icon="⏳" onClick={() => navigateTo('renewals')} />
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="card px-5 sm:px-6 py-5" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px' }}>
          ...
          <div className="flex flex-col gap-4">
            {dashboardData.topDepts.map((dept: any, idx: number) => {
              const max = dashboardData.topDepts[0]?.count || 1;
              const percentage = (dept.count / max) * 100;
              const deptEmps = dashboardData.filteredEmps.filter((e: any) => getDepartment(e) === dept.name);

              return (
                <div key={idx} onClick={() => setSelectedDeptDetails({ name: dept.name, emps: deptEmps })} className="cursor-pointer group p-1.5 rounded-lg hover:bg-slate-50 transition-all">
        ...
        <div className="card px-5 sm:px-6 py-5 flex flex-col lg:col-span-2" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px' }}>
          ...
          <div className="flex-1 flex items-end gap-2 sm:gap-4 h-[150px] pb-4 border-b justify-center" style={{ borderColor: '#e2e8f0' }}>
            {dashboardData.contractsByMonth.filter((m: any) => m.count > 0).length === 0 ? (
              ...
              dashboardData.contractsByMonth
                ...
                .map((month: any, idx: number) => {
                  const height = maxMonthCount > 0 ? (month.count / maxMonthCount) * 100 : 0;
                  return (
                    <div
                      key={idx}
                      onClick={() => setSelectedMonthDetails({ name: month.name, emps: month.emps })}
                      className="flex-1 flex flex-col items-center justify-end h-full cursor-pointer group"
                      style={{ maxWidth: '60px' }}
                    >
                      <span className="text-[11px] font-mono font-bold mb-1" style={{ color: '#2563eb' }}>
                      ...
                      <div
                        ...
                        style={{ height: `${height}%`, minHeight: '4px', background: 'linear-gradient(180deg, #3b82f6, #1d4ed8)' }}
                      />
                      <span className="text-[11px] font-bold mt-2" style={{ color: '#0f172a' }}>
                        {month.name}
                      </span>
                    </div>
                  );
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="card px-5 sm:px-6 py-5 flex flex-col justify-center items-center relative lg:col-span-1" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px' }}>
          ...
          <div style={{ width: '160px', height: '160px', borderRadius: '50%', background: donutGradient, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.06)' }}>
            <div style={{ width: '105px', height: '110px', background: '#ffffff', borderRadius: '50%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: '10px', color: '#64748b', fontWeight: 'bold' }}>إجمالي الهيكل</span>
              <span style={{ fontSize: '18px', fontWeight: '900', color: '#0f172a' }}>{totalContracts.toLocaleString('en-US')}</span>
            </div>
          </div>
              ...
              <table className="data-table" style={{ width: '100%' }}>
                ...
                <tbody>
                  {dashboardData.urgentAlerts.map((alert: any) => {
                    const code = getEmployeeCode(alert);
                    return (
                      <tr key={alert.id || code} onClick={() => handleRowClick(code, 'contracts', alert)} className="cursor-pointer hover:bg-slate-50 transition-colors">
                        <td className="font-mono font-bold" style={{ color: '#2563eb' }}>{code}</td>
                        <td className="font-bold">{getEmployeeName(alert)}</td>
                        <td style={{ color: '#64748b', fontSize: '11px' }}>{getDepartment(alert)}</td>
                        <td className="font-mono font-bold">{getContractEnd(alert)}</td>
                        <td>
            ...
            {dashboardData.missingDataList.length === 0 ? (
              ...
              <div className="table-responsive">
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
                  ...
                  <tbody>
                    {dashboardData.missingDataList.map((emp: any, idx: number) => {
                      const empCode = getEmployeeCode(emp);
                      const empName = getEmployeeName(emp);
                      const nationalId = getNationalId(emp);
                      const mobile = getMobile(emp);
                      return (
                        <tr key={getEmployeeId(emp) || empCode || idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          ...
                          <td style={{ padding: '10px', color: '#dc2626', fontWeight: 'bold' }}>
                            {!nationalId && <span>الرقم القومي </span>}
                            {!mobile && <span style={{ marginRight: '8px' }}>{!nationalId ? '' : '- '}الموبايل</span>}
                          </td>
                          <td style={{ padding: '10px', textAlign: 'center' }}>
                            <button onClick={() => handleRowClick(empCode, 'employees', emp)} style={{ background: '#2563eb', color: '#ffffff', border: 0, padding: '6px 12px', borderRadius: '6px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>تحديث السجل ✏️</button>
                          </td>
                        </tr>
                      );
                    })}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '820px', maxWidth: '100%', height: '80vh', background: '#ffffff', borderRadius: '16px', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setSelectedDeptDetails(null)} style={{ background: '#fef2f2', border: 0, color: '#dc2626', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>إغلاق ✕</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
                ...
                <tbody>
                  {selectedDeptDetails.emps.map((emp: any) => {
                    const empCode = getEmployeeCode(emp);
                    return (
                      <tr key={getEmployeeId(emp) || empCode} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '10px', fontWeight: 'bold', fontFamily: 'monospace', color: '#2563eb' }}>{empCode}</td>
                        <td style={{ padding: '10px', fontWeight: 'bold' }}>{getEmployeeName(emp)}</td>
                        <td style={{ padding: '10px', color: '#64748b' }}>{getJobTitle(emp) || '—'}</td>
                        <td style={{ padding: '10px', fontWeight: 'bold' }}>{getContractType(emp) || '—'}</td>
                        <td style={{ padding: '10px', fontFamily: 'monospace' }}>{getContractEnd(emp) || '—'}</td>
                        <td style={{ padding: '10px', textAlign: 'center' }}>
                          <button onClick={() => handleRowClick(empCode, 'contracts', emp)} style={{ background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', padding: '5px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>عرض العقد ↗️</button>
                        </td>
                      </tr>
                    );
                  })}
      ...
      {showAgeModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '820px', maxWidth: '100%', height: '85vh', background: '#ffffff', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              <button type="button" onClick={() => setAgeModalFilterMode('60days')} style={{ padding: '7px 12px', border: 0, borderRadius: '6px', background: ageModalFilterMode === '60days' ? '#d97706' : '#f8fafc', color: ageModalFilterMode === '60days' ? '#fff' : '#0f172a', fontWeight: 'bold', cursor: 'pointer' }}>الـ 60 يوماً القادمة</button>
              <button type="button" onClick={() => setAgeModalFilterMode('byMonth')} style={{ padding: '7px 12px', border: 0, borderRadius: '6px', background: ageModalFilterMode === 'byMonth' ? '#d97706' : '#f8fafc', color: ageModalFilterMode === 'byMonth' ? '#fff' : '#0f172a', fontWeight: 'bold', cursor: 'pointer' }}>🗓️ اختيار شهر وسنة</button>
              <button type="button" onClick={() => setAgeModalFilterMode('allYear')} style={{ padding: '7px 12px', border: 0, borderRadius: '6px', background: ageModalFilterMode === 'allYear' ? '#d97706' : '#f8fafc', color: ageModalFilterMode === 'allYear' ? '#fff' : '#0f172a', fontWeight: 'bold', cursor: 'pointer' }}>📅 كامل السنة</button>
              {(ageModalFilterMode === 'byMonth' || ageModalFilterMode === 'allYear') && (
                <>
                  {ageModalFilterMode === 'byMonth' && (
                    <select value={ageModalSelectedMonth} onChange={(e) => setAgeModalSelectedMonth(e.target.value)} style={{ padding: '7px 10px', border: '1px solid #e2e8f0', borderRadius: '6px', fontWeight: 'bold' }}>
                      <option value="">كل الأشهر</option>
                      {MONTHS_LIST.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
                    </select>
                  )}
                  <select value={ageModalSelectedYear} onChange={(e) => setAgeModalSelectedYear(e.target.value)} style={{ padding: '7px 10px', border: '1px solid #e2e8f0', borderRadius: '6px', fontWeight: 'bold' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              {displayTurning60List.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px', color: '#64748b', fontWeight: 'bold' }}>لا يوجد موظفون مطابقون.</div>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                  <thead>
                    <tr style={{ background: '#f8fafc' }}>
                      ...
                      <th style={{ padding: '10px' }}>الميلاد</th>
                      <th style={{ padding: '10px' }}>بلوغ 60</th>
                      <th style={{ padding: '10px' }}>الحالة</th>
                      <th style={{ padding: '10px' }}>إجراء</th>
                    </tr>
                  </thead>
                  <tbody>
                    {displayTurning60List.map((emp: any) => (
                      <tr key={getEmployeeId(emp) || getEmployeeCode(emp)}>
                        <td style={{ padding: '10px' }}>{getEmployeeCode(emp)}</td>
                        <td style={{ padding: '10px' }}>{getEmployeeName(emp)}</td>
                        <td style={{ padding: '10px' }}>{getDepartment(emp)}</td>
                        <td style={{ padding: '10px' }}>{emp.birthDate}</td>
                        <td style={{ padding: '10px' }}>{emp.age60Date}</td>
                        <td style={{ padding: '10px' }}>{emp.daysLeft < 0 ? 'بلغ' : `متبقي ${emp.daysLeft} يوم`}</td>
                        <td style={{ padding: '10px' }}>
                          <button onClick={() => handleRowClick(getEmployeeCode(emp), 'contracts', emp)} style={{ background: '#2563eb', color: '#fff', border: 0, padding: '5px 10px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', fontSize: '10px' }}>العقد ↗️</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '800px', maxWidth: '100%', height: '80vh', background: '#fff', borderRadius: '16px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '20px 24px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setSelectedMonthDetails(null)} style={{ background: '#fef2f2', color: '#dc2626', border: 0, padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>إغلاق ✕</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                <thead>
                ...
                <tbody>
                  {selectedMonthDetails.emps.map((emp: any) => (
                    <tr key={getEmployeeId(emp) || getEmployeeCode(emp)} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      <td style={{ padding: '10px' }}>{getEmployeeCode(emp)}</td>
                      <td style={{ padding: '10px' }}>{getEmployeeName(emp)}</td>
                      <td style={{ padding: '10px' }}>{getDepartment(emp)}</td>
                      <td style={{ padding: '10px' }}>{getContractType(emp)}</td>
                      <td style={{ padding: '10px' }}>{getContractStart(emp) || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '800px', maxWidth: '100%', height: '80vh', background: '#fff', borderRadius: '16px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '20px 24px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button onClick={() => setSelectedDonutDetails(null)} style={{ background: '#fef2f2', border: 0, color: '#dc2626', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>إغلاق ✕</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              {selectedDonutDetails.emps.length === 0 ? (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
                  ...
                  <tbody>
                    {selectedDonutDetails.emps.map((emp: any) => {
                      const code = getEmployeeCode(emp);
                      return (
                        <tr key={getEmployeeId(emp) || code} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          <td style={{ padding: '10px', fontWeight: 'bold', fontFamily: 'monospace', color: '#2563eb' }}>{code}</td>
                          <td style={{ padding: '10px', fontWeight: 'bold' }}>{getEmployeeName(emp)}</td>
                          <td style={{ padding: '10px', color: '#64748b' }}>{getDepartment(emp)}</td>
                          <td style={{ padding: '10px', color: '#64748b' }}>{getJobTitle(emp) || '—'}</td>
                          <td style={{ padding: '10px', fontWeight: 'bold' }}>{getContractType(emp) || '—'}</td>
                          <td style={{ padding: '10px', textAlign: 'center' }}>
                            <button onClick={() => handleRowClick(code, 'contracts', emp)} style={{ background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', padding: '5px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>عرض العقد ↗️</button>
                          </td>
                        </tr>
                      );
                    })}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '800px', maxWidth: '100%', height: '80vh', background: '#ffffff', borderRadius: '16px', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowTotalEmpsModal(false)} style={{ background: '#fef2f2', border: 0, color: '#dc2626', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '11px' }}>إغلاق ✕</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
                <thead>
                ...
                <tbody>
                  {dashboardData.filteredEmps.map((emp: any) => {
                    const empCode = getEmployeeCode(emp);
                    return (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '800px', maxWidth: '100%', height: '80vh', background: '#ffffff', borderRadius: '16px', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowExpiringSoonModal(false)} style={{ background: '#fef2f2', border: 0, color: '#dc2626', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '11px' }}>إغلاق ✕</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              {dashboardData.expiringSoonList.length === 0 ? (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
                  ...
                  <tbody>
                    {dashboardData.expiringSoonList.map((emp: any) => {
                      const empCode = getEmployeeCode(emp);
                      return (
                        <tr key={empCode} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          <td style={{ padding: '10px', fontWeight: 'bold', fontFamily: 'monospace', color: '#2563eb' }}>{empCode}</td>
                          ...
                          <td style={{ padding: '10px', fontWeight: 'bold' }}>
                            {getContractType(emp)} {emp.isPermanentTurning60 && <span style={{ fontSize: '9px', background: '#fffbe1', color: '#b45309', padding: '2px 6px', borderRadius: '4px' }}>بلوغ 60</span>}
                          </td>
                          <td style={{ padding: '10px', fontFamily: 'monospace', fontWeight: 'bold' }}>{getContractEnd(emp) || '—'}</td>
                          <td style={{ padding: '10px', textAlign: 'center' }}>
                            <span style={{ color: '#d97706', fontWeight: 'bold' }}>متبقي {emp.days} يوم</span>
                          </td>
                          <td style={{ padding: '10px', textAlign: 'center' }}>
                            <button onClick={() => handleRowClick(empCode, 'contracts', emp)} style={{ background: '#fffbe1', color: '#b45309', border: '1px solid #fde68a', padding: '5px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>تجديد العقد ↗️</button>
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div style={{ padding: '20px 24px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {selectedShortTermDept && (
                  <button onClick={() => setSelectedShortTermDept(null)} style={{ background: '#e2e8f0', border: 0, color: '#0f172a', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '11px' }}>🔙 رجوع للإدارات</button>
                )}
                <button onClick={() => { setShowShortTermModal(false); setSelectedShortTermDept(null); }} style={{ background: '#fef2f2', border: 0, color: '#dc2626', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '11px' }}>إغلاق ✕</button>
              </div>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              {!selectedShortTermDept ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {dashboardData.shortTermList.length === 0 ? (
                    ...
                    dashboardData.shortTermList.map((group: any, idx: number) => (
                ...
                <div className="table-responsive">
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
                    ...
                    <tbody>
                      {dashboardData.shortTermList.find((g: any) => g.deptName === selectedShortTermDept)?.emps.map((emp: any) => {
                        const empCode = getEmployeeCode(emp);
                        const daysLeft = getDaysRemaining(getContractEnd(emp));
                        return (
                          <tr key={empCode} style={{ borderBottom: '1px solid #f1f5f9' }}>
                            <td style={{ padding: '10px' }}>
                              <div style={{ fontWeight: 'bold', color: '#0f172a' }}>{getEmployeeName(emp)}</div>
                            ...
                            <td style={{ padding: '10px' }}>
                              <div style={{ fontFamily: 'monospace', fontWeight: 'bold', color: '#0f172a' }}>{getContractEnd(emp)}</div>
                            ...
                            <td style={{ padding: '10px', textAlign: 'center' }}>
                              <button onClick={() => handleRowClick(empCode, 'contracts', emp)} style={{ background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', padding: '6px 12px', borderRadius: '6px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>العقد ↗️</button>
                            </td>
                          </tr>
                        );
                      })}
