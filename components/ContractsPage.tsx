...
export default function ContractsPage() {
  ...
  // ✅ التحقق من التواريخ المدخلة يدوياً فقط (عشان يرفض 027 و 222027)
  const validateInputDate = (dateStr: string) => {
  ...
  // دوالك الأصلية بدون تغييرات بوظت الداتا
  ...
  const calculateNewEndDate = (oldDateStr: string | undefined, months: number) => {
    if (!oldDateStr) return null; // ✅ تم التعديل لـ null بدل '' لمنع خطأ قاعدة البيانات
  ...
  // الفلترة الأصلية الخاصة بك مع دعم الكروت المجمعة
  ...
  const sortedContracts = [...filteredContracts].sort((a, b) => {
    ...
    let valA = key === 'days_remaining' ? (getDaysRemaining(a.contract_end_date) ?? 99999) : (a[key] ? String(a[key]).toLowerCase() : '');
    let valB = key === 'days_remaining' ? (getDaysRemaining(b.contract_end_date) ?? 99999) : (b[key] ? String(b[key]).toLowerCase() : '');
    if (valA < valB) return direction === 'asc' ? -1 : 1;
    if (valA > valB) return direction === 'asc' ? 1 : -1;
    return 0;
  });

  // الإحصائيات الخاصة بالكروت
  ...
  const handleCreateBrandNewContract = async (e: React.FormEvent) => {
    ...
    // ✅ الحماية شغالة هنا وقت الإدخال فقط!
    if (!validateInputDate(newContractStartDate) || !validateInputDate(newContractEndDate)) {
      return alert('يرجى كتابة تاريخ صحيح وسنة منطقية (أربعة أرقام مثل 2024).');
    }
    ...
    const payload: any = {
      ...
      contract_start_date: newContractStartDate || null, // ✅ حماية الداتابيز
      contract_end_date: newContractStartDate || null,   // ✅ حماية الداتابيز
      new_contract_end_date: newContractEndDate || null, // ✅ حماية الداتابيز
      status: 'Pending',
  ...
  const confirmRenewalAction = async () => {
    // ✅ الحماية على التاريخ المخصص هنا فقط
    if (renewalMode === 'custom' && !validateInputDate(customEndDate)) {
      return alert('يرجى إدخال تاريخ انتهاء مخصص صحيح وسنة من 4 أرقام.');
    }
    setActionLoading(true);
    ...
    if (modalState.type === 'single' && modalState.emp) {
      ...
      const payload: any = {
        ...
        job_title: emp.job_title || null,
        company: emp.company || null,
        contract_end_date: emp.contract_end_date || null, // ✅ حماية الداتابيز
        new_contract_end_date: targetEndDate || null,     // ✅ حماية الداتابيز
    ...
    return (
      <th onClick={() => handleSort(sortKey)} style={{ padding: '12px 10px', background: isSorted ? '#e2e8f0' : '#f8fafc', borderBottom: '1px solid var(--line)', color: isSorted ? '#0f172a' : '#475569', cursor: 'pointer', userSelect: 'none', textAlign: align }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
          <span>{label}</span>
          <span style={{ fontSize: '10px', color: isSorted ? '#2563eb' : '#a1a1aa' }}>{isSorted ? (direction === 'asc' ? '▲' : '▼') : '↕'}</span>
        </div>
      </th>
    );
  };
    ...
    <div style={{ paddingBottom: '40px', direction: 'rtl' }}>
      ...
      <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '11px', color: 'var(--muted)' }}>سجل شامل لعقود الموظفين مع إحصائيات ERP التفاعلية</p>
        </div>
          ...
          <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
            ...
            <tbody>
              {sortedContracts.length === 0 ? (
                <tr><td colSpan={10} style={{ textAlign: 'center', padding: '30px', color: 'var(--muted)', fontWeight: 'bold' }}>لا توجد بيانات متطابقة</td></tr>
              ) : (
          ...
          <div style={{ width: '520px', background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <form onSubmit={handleCreateBrandNewContract}>
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '11px', color: 'var(--muted)', marginBottom: '6px', fontWeight: 'bold' }}>ابحث عن الموظف (بالاسم أو الكود) *</label>
                <input type="text" list="new-contract-employees" required placeholder="🔍 اكتب كود أو اسم الموظف هنا..." value={empSearchTerm} onChange={(e) => { const val = e.target.value; setEmpSearchTerm(val); const code = val.split(' - ')[0]; const isValidEmp = employees.some(emp => emp.employee_code === code && emp.contract_type !== 'إنهاء تعاقد'); if (isValidEmp) setSelectedEmployeeCode(code); else setSelectedEmployeeCode(''); }} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '12px', outline: 'none', fontWeight: 'bold', background: '#f8fafc' }} />
        ...
        <div className="no-print" style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
          <div style={{ width: '500px', background: '#fff', borderRadius: '16px', padding: '28px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ background: '#fdfbf7', border: '1px solid #f1e9d2', borderRadius: '12px', padding: '12px 20px', marginBottom: '20px', display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 'bold', color: renewalMode === 'months' ? '#856404' : '#64748b', cursor: 'pointer' }}><input type="radio" name="renewalMode" checked={renewalMode === 'months'} onChange={() => setRenewalMode('months')} style={{ accentColor: '#b8934a' }} /> تجديد بالشهور (تلقائي)</label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 'bold', color: renewalMode === 'custom' ? '#856404' : '#64748b', cursor: 'pointer' }}><input type="radio" name="renewalMode" checked={renewalMode === 'custom'} onChange={() => setRenewalMode('custom')} style={{ accentColor: '#b8934a' }} /> تاريخ انتهاء مخصص</label>
            </div>
            {renewalMode === 'months' && (
              <div style={{ marginBottom: '20px' }}>
            ...
            {modalState.type === 'single' && (
              <div style={{ textAlign: 'left', fontSize: '12px', marginBottom: '24px', direction: 'ltr' }}>
                <span style={{ color: '#15803d', fontWeight: 'bold', fontFamily: 'monospace', fontSize: '13px' }}>{renewalMode === 'months' ? (calculateNewEndDate(modalState.emp?.contract_end_date, renewalMonths) || '—') : (customEndDate || '—')}</span>
                <span style={{ color: '#64748b', fontWeight: 'bold', marginLeft: '6px' }}>:تاريخ الانتهاء المتوقع</span>
              </div>
            )}
            <div style={{ display: 'flex', justifyContent: 'flex-start', gap: '10px' }}>
              <button onClick={confirmRenewalAction} disabled={actionLoading} style={{ background: '#b8934a', color: '#fff', border: 0, padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', fontSize: '12px', cursor: actionLoading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>✅ {actionLoading ? 'جاري التنفيذ...' : 'تأكيد وإجراء الطلب'}</button>
