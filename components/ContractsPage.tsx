...
export default function ContractsPage() {
  ...
  // 🔍 الفلاتر الأساسية (التي طلب المستخدم الإبقاء عليها)
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedDept, setSelectedDept] = useState('');
  const [selectedType, setSelectedType] = useState('');
  const [expiryMonth, setExpiryMonth] = useState(''); 
  
  // 🗂️ فلتر الكروت العلوية (المدمجة: الأساسية + الـ Workflow)
  ...
  // 📝 النوافذ المنبثقة (الخاصة بالمستخدم)
  ...
  // 🚀 حالات نافذة الـ Workflow الجديدة (متابعة الطلب)
  ...
  // 📊 حسابات كروت الـ Workflow الجديدة
  ...
  const filteredContracts = useMemo(() => {
    return activeEmployees.filter((emp) => {
      ...
      if (activeFilterCard === 'fixed') return emp.contract_type?.includes('محدد');
      if (activeFilterCard === 'overage') return emp.contract_type?.includes('فوق السن');
  ...
  // دوال المستخدم الأصلية (تحديد، حذف، إضافة، تعديل)
  ...
  // 🚀 دوال الـ Workflow (متابعة واعتماد الطلبات)
  ...
  return (
    <div style={{ paddingBottom: '40px', animation: 'fadeIn 0.4s ease-in-out', direction: 'rtl', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      {/* الهيدر والزراير العلوية الأصلية */}
      ...
      {/* 🗂️ الكروت التفاعلية الجديدة (بتصميم مختلف وألوان Indigo/Violet) */}
      ...
      {workflowModal.isOpen && workflowModal.req && (
        <div className="no-print" style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '450px', background: '#ffffff', borderRadius: '20px', padding: '28px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', direction: 'rtl' }}>
            ...
            {/* حالة: قيد الانتظار (تحتاج اعتماد أو رفض) */}
            ...
            {workflowModal.req.status === 'Approved' && workflowModal.req.signature_status !== 'تم التوقيع' && (
              <div>
                ...
                <p style={{ fontSize: '10px', color: '#94a3b8', textAlign: 'center', marginTop: '8px' }}>
                  بالضغط هنا، سيتم أرشفة العقد القديم وتحديث تاريخ نهاية عقد الموظف بالبيانات الجديدة أوتوماتيكياً.
                </p>
              </div>
            )}
          </div>
      ...
      {/* باقي النوافذ الأصلية (طلب تجديد فردي ومجمع، تعديل مباشر، إنهاء) موجودة بالأسفل دون مساس */}
      ...
      {/* نافذة التعديل المباشر والتيرمينيت ... موجودة كالسابق */}
        ...
        <div className="no-print" style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '480px', background: '#ffffff', borderRadius: '20px', padding: '28px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', direction: 'rtl' }}>
            ...
            <form onSubmit={handleTerminateContract}>
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '12px', color: '#64748b', marginBottom: '8px', fontWeight: 'bold' }}>البحث عن الموظف (بالكود أو الاسم) *</label>
