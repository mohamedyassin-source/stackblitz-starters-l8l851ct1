...
export default function EmployeesPage() {
  ...
  const deptsList = useMemo(() => {
    return Array.from(new Set(activeEmployeesOnly.map((emp: any) => getField(emp, 'department', 'Department')).filter(Boolean)));
  }, [activeEmployeesOnly]);
  ...
  const renderSortArrow = (column: string) => {
    ...
    return <span style={{ marginRight: '4px', color: 'var(--brass-600, #0d9488)' }}>{sortDirection === 'asc' ? '▲' : '▼'}</span>;
  };
  ...
  const getContractStatusBadge = (type: string, endDate: string) => {
    if (endDate && String(endDate).trim()) {
      ...
      if (days < 0) return <span style={{ background: 'var(--stamp-red-bg)', color: 'var(--stamp-red)', padding: '3px 8px', borderRadius: '6px', fontWeight: 'bold', fontSize: '10px' }}>{endDate} 🚨</span>;
  ...
  const renderAgeBadge = (emp: any) => {
    ...
    if (age >= 60) return <span style={{ background: 'var(--stamp-amber-bg)', color: 'var(--stamp-amber)', padding: '3px 8px', borderRadius: '6px', fontWeight: 'bold', fontSize: '10px' }}>💼 {age} سنة (60+)</span>;
    ...
    <div style={{ direction: 'rtl', animation: 'fadeIn 0.4s ease-in-out' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setShowAddModal(true)} style={{ background: 'var(--brass-600, #0d9488)', color: '#fff', border: 0, padding: '7px 14px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>+ إضافة موظف</button>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '20px' }}>
        <div onClick={() => setActiveCardFilter(activeCardFilter === 'ALL_ACTIVE' ? null : 'ALL_ACTIVE')} style={{ background: activeCardFilter === 'ALL_ACTIVE' ? '#f0fdf4' : 'var(--paper-card)', border: activeCardFilter === 'ALL_ACTIVE' ? '2px solid #22c55e' : '1px solid var(--line)', padding: '12px 16px', borderRadius: '12px', cursor: 'pointer' }}>
          <div style={{ fontSize: '11px', fontWeight: 'bold' }}>إجمالي الموظفين</div>
        ...
        <div onClick={() => setActiveCardFilter(activeCardFilter === 'ABOVE_AGE' ? null : 'ABOVE_AGE')} style={{ background: activeCardFilter === 'ABOVE_AGE' ? '#fef3c7' : 'var(--paper-card)', border: activeCardFilter === 'ABOVE_AGE' ? '2px solid #d97706' : '1px solid var(--line)', padding: '12px 16px', borderRadius: '12px', cursor: 'pointer' }}>
          ...
          <div style={{ fontSize: '10px', color: 'var(--stamp-amber)', fontWeight: 'bold' }}>{kpiStats.aboveAgePct}% من القوة الحالية</div>
        </div>
      </div>
      ...
      {selectedEmpIds.length > 0 && (
        <div style={{ background: '#0f172a', color: '#fff', padding: '10px 16px', borderRadius: '10px', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: '11px', fontWeight: 'bold' }}>تم تحديد <span style={{ color: '#38bdf8' }}>{selectedEmpIds.length}</span> موظف</div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => setShowBulkTransferModal(true)} style={{ padding: '6px 12px', background: '#2563eb', color: '#fff', border: 0, borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>نقل مجمع 🔄</button>
            <button onClick={() => handleExportToExcel(true)} style={{ padding: '6px 12px', background: '#16a34a', color: '#fff', border: 0, borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>تصدير المحدد 📥</button>
            <button onClick={handleDeleteSelected} disabled={isDeleting} style={{ padding: '6px 12px', background: '#dc2626', color: '#fff', border: 0, borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>{isDeleting ? 'جاري الحذف...' : 'حذف نهائي 🗑️'}</button>
            <button onClick={() => setSelectedEmpIds([])} style={{ padding: '6px 12px', background: 'transparent', color: '#fff', border: '1px solid #64748b', borderRadius: '6px', cursor: 'pointer' }}>إلغاء ✕</button>
          </div>
        </div>
      )}

      <div style={{ background: 'var(--paper-card)', border: '1px solid var(--line)', padding: '12px 16px', borderRadius: '12px', marginBottom: '20px', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
        <input type="text" placeholder="بحث بالاسم، الكود، الرقم القومي، الإدارة..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '11px', minWidth: '260px', outline: 'none' }} />
        <input list="deptList" placeholder="الإدارة..." value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '11px', width: '130px' }} />
        ...
        <button onClick={() => { setSearchTerm(''); setSelectedDept(''); setSelectedCompany(''); setSelectedType(''); setSelectedAgeRange(''); setActiveCardFilter('ALL_ACTIVE'); }} style={{ background: 'var(--line)', border: 0, padding: '8px 14px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>إعادة ضبط</button>
        <div style={{ marginRight: 'auto', fontSize: '11px', fontWeight: 'bold' }}>النتائج: {finalTableEmployees.length.toLocaleString('en-US')}</div>
      </div>

      <div style={{ background: 'var(--paper-card)', border: '1px solid var(--line)', borderRadius: '12px', overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: '60px', textAlign: 'center', fontWeight: 'bold', color: 'var(--muted)' }}>جاري تحميل بيانات الموظفين... ⏳</div>
        ) : (
          <div style={{ maxHeight: '55vh', overflowY: 'auto' }}>
            <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse', whiteSpace: 'nowrap', fontSize: '11.5px' }}>
              <thead style={{ position: 'sticky', top: 0, background: 'var(--paper-card)', zIndex: 10 }}>
                <tr>
                  ...
                  <th onClick={() => handleSort('department')} style={{ padding: '12px', cursor: 'pointer' }}>الإدارة</th>
              ...
              <tbody>
                {finalTableEmployees.map((emp: any) => {
                  ...
                  return (
                    <tr key={employeeId || code} style={{ borderBottom: '1px solid var(--line)' }}>
                      ...
                      <td style={{ padding: '10px', fontWeight: 'bold', color: 'var(--brass-600, #0d9488)', fontFamily: 'monospace' }}>{code}</td>
                      <td style={{ padding: '10px', fontWeight: 'bold' }}>{getEmployeeName(emp)} {missing && (<span title="ناقص الرقم القومي أو الموبايل" style={{ marginRight: '6px' }}>⚠️</span>)}</td>
                      <td style={{ padding: '10px', color: 'var(--muted)' }}>{getField(emp, 'job_title', 'JobTitle') || '—'}</td>
                      <td style={{ padding: '10px', color: 'var(--muted)' }}>{getField(emp, 'department', 'Department') || '—'}</td>
                      ...
                      <td style={{ padding: '10px', fontWeight: 'bold' }}>{contractType || '—'}</td>
                      <td style={{ padding: '10px' }}>{getContractStatusBadge(contractType, contractEnd)}</td>
                      <td style={{ padding: '10px', textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                          <button onClick={() => setProfileEmp(emp)} style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid var(--line)', cursor: 'pointer', fontSize: '10px' }}>👁️ الملف</button>
                          <button onClick={() => handleOpenEdit(emp)} style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid var(--line)', background: 'transparent', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>تعديل ✏️</button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '650px', maxWidth: '100%', background: '#fff', borderRadius: '16px', padding: '24px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginTop: '20px', textAlign: 'left' }}>
              <button onClick={() => { handleOpenEdit(profileEmp); setProfileEmp(null); }} style={{ background: 'var(--brass-600, #0d9488)', color: '#fff', border: 0, padding: '8px 16px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>تعديل البيانات ✏️</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showTermModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '550px', maxWidth: '100%', background: '#fff', borderRadius: '16px', padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '18px' }}>
              <h3 style={{ margin: 0, color: 'var(--stamp-red)' }}>🚫 إنهاء خدمة</h3>
              <button onClick={() => { setShowTermModal(false); setSelectedTermEmp(null); setTermSearch(''); }} style={{ background: '#fef2f2', color: '#dc2626', border: 0, padding: '6px 12px', borderRadius: '6px', cursor: 'pointer' }}>إغلاق ✕</button>
            </div>
            <form onSubmit={handleConfirmTermination} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <input placeholder="كود أو اسم أو رقم قومي..." value={termSearch} onChange={(e) => { setTermSearch(e.target.value); setSelectedTermEmp(null); }} style={{ padding: '10px', borderRadius: '8px', border: '1px solid var(--line)' }} />
              {termSearchResults.length > 0 && !selectedTermEmp && (
                <div style={{ border: '1px solid var(--line)', borderRadius: '8px', maxHeight: '180px', overflowY: 'auto' }}>
                  {termSearchResults.map((emp: any, index: number) => (
                    <div key={getEmployeeId(emp) || index} onClick={() => { setSelectedTermEmp(emp); setTermSearch(`${getEmployeeCode(emp)} - ${getEmployeeName(emp)}`); }} style={{ padding: '10px', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }}>
                      <strong>[{getEmployeeCode(emp)}]</strong> {getEmployeeName(emp)}
                      <div style={{ fontSize: '10px', color: '#64748b' }}>الرقم القومي: {getNationalId(emp) || '—'}</div>
                    </div>
                  ))}
                </div>
              )}
              {selectedTermEmp && (
                <div style={{ background: '#fef2f2', padding: '10px', borderRadius: '8px', color: '#dc2626', fontWeight: 'bold' }}>
                  الموظف المحدد: {getEmployeeName(selectedTermEmp)} {' — '} {getEmployeeCode(selectedTermEmp)}
                </div>
              )}
              <select value={termReason} onChange={(e) => setTermReason(e.target.value)} style={{ padding: '9px', borderRadius: '8px', border: '1px solid var(--line)' }}>
              ...
              <input type="date" value={termDate} onChange={(e) => setTermDate(e.target.value)} required style={{ padding: '9px', borderRadius: '8px', border: '1px solid var(--line)' }} />
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
                <button type="button" onClick={() => setShowTermModal(false)} style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--line)' }}>إلغاء</button>
                <button type="submit" disabled={termSaving || !selectedTermEmp} style={{ background: 'var(--stamp-red)', color: '#fff', border: 0, padding: '8px 16px', borderRadius: '8px', fontWeight: 'bold' }}>{termSaving ? 'جاري الحفظ...' : 'تأكيد التحويل'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showBulkTransferModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '500px', maxWidth: '100%', background: '#fff', borderRadius: '16px', padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h3 style={{ margin: 0 }}>🔄 النقل المجمع</h3>
              <button onClick={() => setShowBulkTransferModal(false)}>إغلاق ✕</button>
            </div>
            <form onSubmit={handleConfirmBulkTransfer} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <input list="bulkDeptList" placeholder="الإدارة الجديدة" value={bulkDept} onChange={(e) => setBulkDept(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid var(--line)' }} />
              <datalist id="bulkDeptList">{deptsList.map((d: any, i: number) => (<option key={i} value={d} />))}</datalist>
              <input list="bulkCompList" placeholder="الشركة الجديدة" value={bulkCompany} onChange={(e) => setBulkCompany(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid var(--line)' }} />
              <datalist id="bulkCompList">{compsList.map((c: any, i: number) => (<option key={i} value={c} />))}</datalist>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
                <button type="button" onClick={() => setShowBulkTransferModal(false)}>إلغاء</button>
                <button type="submit" disabled={bulkSaving} style={{ background: '#2563eb', color: '#fff', border: 0, padding: '9px 16px', borderRadius: '8px', fontWeight: 'bold' }}>{bulkSaving ? 'جاري التحديث...' : 'تأكيد النقل'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div style={{ width: '700px', maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto', background: '#fff', borderRadius: '16px', padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h3 style={{ margin: 0 }}>إضافة موظف جديد</h3>
              <button onClick={() => setShowAddModal(false)}>إغلاق ✕</button>
            </div>
            <form onSubmit={handleAddEmployee}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <input required placeholder="كود الموظف *" value={newEmp.employee_code} onChange={(e) => setNewEmp({ ...newEmp, employee_code: e.target.value })} />
                <input required placeholder="اسم الموظف *" value={newEmp.employee_name} onChange={(e) => setNewEmp({ ...newEmp, employee_name: e.target.value })} />
                <input placeholder="الرقم القومي" value={newEmp.national_id} onChange={(e) => setNewEmp({ ...newEmp, national_id: e.target.value })} />
                <input type="date" value={newEmp.birth_date} onChange={(e) => setNewEmp({ ...newEmp, birth_date: e.target.value })} />
                <input placeholder="الإدارة" value={newEmp.department} onChange={(e) => setNewEmp({ ...newEmp, department: e.target.value })} />
